...
export class UserPreferencesService {
    ...
    constructor(
        ...
        combineLatest([this.authQuery.authenticated$, this.appSettingsQuery.selectEntity(AppSettings.apiHome)]).subscribe(
            ([authenticated, apiHome]) => {
    ...
    public setValue(key: string, value: any): Observable<any> {
        return new Observable<any>(observer => {
            this.http.post(`${this.appSettings.getValue(AppSettings.apiHome)}/api/v1/userpreferencevalue`, { name: key, value }).subscribe(
                () => {
    ...
    private getDefinitions() {
        this.httpInFlight++;
        this.http
            .get<Array<UserPreferenceDefinition>>(`${this.appSettings.getValue(AppSettings.apiHome)}/api/v1/UserPreferenceDefinition`)
            .subscribe(
                prefDefinitions => {
