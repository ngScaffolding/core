...
export class ReferenceValuesService {
  ...
  lookupValue(name: string, lookupValue: any): Observable<string> {
    return new Observable<string>(observer => {
        this.getReferenceValue(name)
            .pipe(tap(refVal => {
              const foundVal =refVal.referenceValueItems.find(val => val.value == lookupValue);
              observer.next(refVal.referenceValueItems.find(val => val.value == lookupValue)?.display);
              observer.complete();
            }))
            .subscribe();
    });
  ...
  clearReferenceValue(clearNames: string | Array<string>, notifyOthers = true) {
    ...
    for (const loopName of namesArray) {
      const list = this.refValuesQuery.getAll({ filterBy: entity => entity.name.includes(loopName) });
      if (notifyOthers) {
